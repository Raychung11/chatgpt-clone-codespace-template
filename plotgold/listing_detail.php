...
<nav class="bg-white border-bottom">
    <div class="container py-2">
        <ol class="breadcrumb pg-breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="<?= pg_url('browse_listings.php') ?>"><?= _e('browse.title') ?></a></li>
...
<div class="container py-4">
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="pg-card p-4 mb-4">
                <h5 class="fw-600 mb-3"><?= _e('detail.about_title') ?></h5>
                <p class="text-muted" style="line-height:1.9"><?= nl2br(h($listing['public_description'] ?? __('detail.no_description'))) ?></p>
            </div>

            <!-- Specifications -->
            <div class="pg-card p-4 mb-4">
                <h5 class="fw-600 mb-3"><?= _e('detail.specs_title') ?></h5>
                <div class="row">
                    <div class="col-md-6">
                        <table class="table listing-spec-table">
                            <tr><td><?= _e('detail.type') ?></td><td><?= h($listing['type_label']) ?></td></tr>
                            <tr><td><?= _e('detail.religion') ?></td><td><?= h($listing['religion_label'] ?? '—') ?></td></tr>
                            <tr><td><?= _e('detail.park') ?></td><td><?= $listing['park_name'] ? '<a href="' . park_url($listing['park_slug']) . '">' . h($listing['park_name']) . '</a>' : '—' ?></td></tr>
                            <tr><td><?= _e('detail.section') ?></td><td><?= h($listing['section_name'] ?? '—') ?></td></tr>
                            <tr><td><?= _e('detail.block_row_lot') ?></td><td>
                                <?= implode(' / ', array_filter([h($listing['block_no']), h($listing['row_no']), h($listing['lot_no'])])) ?: '—' ?>
                            </td></tr>
                            <tr><td><?= _e('detail.city') ?></td><td><?= h($listing['city']) ?>, <?= h($listing['state']) ?></td></tr>
                        </table>
                    </div>
                    <div class="col-md-6">
                        <table class="table listing-spec-table">
                            <tr><td><?= _e('detail.price') ?></td><td class="fw-600 text-navy"><?= format_currency($listing['asking_price']) ?></td></tr>
                            <tr><td><?= _e('detail.transfer_fee') ?></td><td><?= $listing['transfer_fee'] ? format_currency($listing['transfer_fee']) : 'TBC' ?></td></tr>
                            <tr><td><?= _e('detail.maintenance_fee') ?></td><td><?= $listing['annual_maintenance_fee'] ? format_currency($listing['annual_maintenance_fee']) : '—' ?></td></tr>
                            <tr><td><?= _e('detail.maintenance') ?></td><td><?= ucfirst($listing['maintenance_status'] ?? '—') ?></td></tr>
                            <tr><td><?= _e('detail.ownership') ?></td><td><?= ucfirst(str_replace('_', ' ', $listing['ownership_type'] ?? '—')) ?></td></tr>
                            <tr><td><?= _e('detail.transferable') ?></td><td><?= $listing['is_transferable'] ? '<span class="text-success fw-500">' . _e('detail.transferable_yes') . '</span>' : '<span class="text-warning fw-500">' . _e('detail.transferable_check') . '</span>' ?></td></tr>
                        </table>
                    </div>
                </div>
                ...
                <div class="bg-pale-gold p-3 rounded-2 mt-2">
                    <h6 class="fw-600 small mb-1"><i class="fas fa-compass me-2 text-gold"></i><?= _e('detail.feng_shui') ?></h6>
            ...
            <!-- Verification Status -->
            <div class="pg-card p-4 mb-4">
                <h5 class="fw-600 mb-3"><i class="fas fa-shield-alt me-2 text-gold"></i><?= _e('detail.verification') ?></h5>
            ...
            <div class="sticky-enquiry">
                ...
                <div class="text-center mb-3 pb-3 border-bottom">
                    <?php if ($listing['asking_price']): ?>
                        ...
                        <div class="small text-muted"><?= _e('detail.price') ?></div>
                        <?php if ($listing['market_estimate']): ?>
                            <div class="small text-muted mt-1"><?= _e('detail.location') ?> Est. <?= format_currency($listing['market_estimate']) ?></div>
                        <?php endif; ?>
                    <?php else: ?>
                        <div class="fs-5 fw-600 text-muted"><?= _e('detail.price_on_enquiry') ?></div>
                    <?php endif; ?>
                </div>
                ...
                <a href="https://wa.me/<?= WHATSAPP_NUMBER ?>?text=<?= $whatsappMsg ?>"
                   target="_blank" rel="noopener" class="btn btn-whatsapp w-100 mb-2 py-3">
                    <i class="fab fa-whatsapp me-2 fs-5"></i><?= _e('detail.whatsapp_cta') ?>
                </a>
                ...
                <form action="<?= pg_url('api/enquiry.php') ?>" method="POST" id="enquiryForm">
                    ...
                    <div class="mb-2">
                        <input type="text" name="contact_name" class="form-control form-control-sm"
                               placeholder="<?= _e('detail.enquiry_name') ?>" required value="<?= auth_check() ? h(auth_user_name()) : '' ?>">
                    </div>
                    <div class="mb-2">
                    ...
                    <button type="submit" class="btn btn-navy w-100 btn-sm">
                        <i class="fas fa-envelope me-2"></i><?= _e('detail.enquiry_btn') ?>
                    </button>
                </form>
                ...
                <div class="border-top mt-3 pt-3">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                            <div class="small fw-500"><?= _e('detail.listed_by') ?> <?= h($listing['seller_name'] ?? 'Verified Seller') ?></div>
                                ...
                                <?php if ($listing['seller_phone']): ?>
                                    <a href="https://wa.me/<?= preg_replace('/[^0-9]/', '', $listing['seller_phone']) ?>?text=<?= $whatsappMsg ?>"
                                       class="text-muted" target="_blank" rel="noopener">
                                        <i class="fab fa-whatsapp me-1"></i><?= _e('detail.contact_seller') ?>
                                    </a>
                                <?php else: ?>
                                    <?= _e('detail.contact_via') ?>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
    ...
    <?php if ($similar): ?>
    <div class="mt-5">
        <h4 class="section-title mb-2"><?= _e('detail.similar') ?></h4>
